...
namespace CarManufacturer
{
    public class StartUp
    {
        static void Main(string[] args)
        {
            ...
            List<Car> filteredCarList = (List<Car>)carList.Where(x => x.Year >= 2017).Where(x => x.Engine.HorsePower > 330).Where(x =>
            {
                double totalPressureSum = 0;
                foreach (var item in x.Tires)
                {
                    totalPressureSum += item.Pressure;
                }
                return totalPressureSum >= 9 && totalPressureSum <= 10;
            }).ToList();
